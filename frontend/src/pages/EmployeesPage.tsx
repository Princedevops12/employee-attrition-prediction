...
export default function EmployeesPage() {
  ...
  const loadEmployees = async () => {
    ...
    setError(null)
    try {
      const data = await apiService.listEmployees(0, 50, departmentFilter || undefined)
      setEmployees(data)
    } catch (err) {
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="mb-2 text-3xl font-bold text-gray-900 dark:text-white">Employees</h1>
      ...
      {showForm && (
        <div className="card">
          <h2 className="mb-4 text-lg font-semibold text-gray-900 dark:text-white">Add New Employee</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                ...
                <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900 dark:text-white">Job Role</th>
                <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900 dark:text-white">Age</th>
                <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900 dark:text-white">Income</th>
                <th className="px-4 py-3 text-left text-sm font-semibold text-gray-900 dark:text-white">Tenure</th>
            ...
            <tbody>
              {filteredEmployees.map(emp => (
                <tr key={emp.id} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{emp.job_role}</td>
                  <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{emp.age}</td>
                  <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">${emp.monthly_income.toLocaleString()}</td>
                  <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{emp.years_at_company} yrs</td>
