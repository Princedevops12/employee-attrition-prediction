...
export const apiService = {
  ...
  async predictBatch(file: File): Promise<{ processed_count: number; success_count: number; error_count: number }> {
    try {
  ...
  // Employees
  async listEmployees(skip = 0, limit = 50, department?: string): Promise<Employee[]> {
    try {
  ...
  async getDepartmentStats(department: string): Promise<any> {
    try {
      const response = await apiClient.get(`/api/employees/department/${department}/stats`)
      return response.data
    } catch (error) {
      throw this.handleError(error)
    }
